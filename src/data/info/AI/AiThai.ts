import {IAiAll} from "@/data/model/ai/IAi";

export const AiThai: IAiAll = {
    fah: {
        title: "ฟ้า เพื่อนคุยเอไอ",
        content:
            "ฟ้า สาวไทยน่ารักและเป็นกันเอง พร้อมเป็นเพื่อนคุยของคุณ ไม่ว่าจะเป็นคำแนะนำในชีวิต การแนะนำสถานที่ท่องเที่ยว หรือแค่พูดคุยสนุกๆ " +
            "ฟ้าพร้อมช่วยเสมอ ฟ้าสื่อสารได้ 5 ภาษา ทั้งภาษาไทย อังกฤษ จีน ญี่ปุ่น และเกาหลี หากคุณมี LINE อยู่แล้ว อย่ารอช้า เพิ่มฟ้าเป็นเพื่อนตอนนี้ " +
            "แล้วสัมผัสประสบการณ์ใหม่ที่น่าประทับใจ!",
        image: {
            src: "nong-fah-0003.jpg",
            alt: "น้องฟ้า เพื่อนคุยเอไอ"
        },
        images: [
            {
                src: "nong-fah-0001.jpg",
                alt: "น้องฟ้า เพื่อนคุยเอไอ"
            },
            {
                src: "nong-fah-0002.jpg",
                alt: "น้องฟ้า เพื่อนคุยเอไอ"
            },
            {
                src: "nong-fah-0003.jpg",
                alt: "น้องฟ้า เพื่อนคุยเอไอ"
            },
            {
                src: "nong-fah-0004.jpg",
                alt: "น้องฟ้า เพื่อนคุยเอไอ"
            }
        ],
        line: {
            link: "https://lin.ee/ODTzbwO",
            button: "https://scdn.line-apps.com/n/line_add_friends/btn/th.png",
        }
    }
}