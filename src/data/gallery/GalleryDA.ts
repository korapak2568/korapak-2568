import {IGallery} from "@/data/gallery/model/IGallery";

export const GalleryDA: IGallery = {
    slides: [
        {
            title: "Fleksible Arbejdsrum",
            description: "Fleksible arbejdspladser efter kundens behov, beliggende i Bangkok, Thailand, tæt på BTS- og MRT-stationer, vigtige vartegn og førende indkøbscentre som Siam Paragon, Central World og ICON Siam. Klar til fjernarbejde, hybridarbejde eller arbejde på stedet i Chiang Mai, Bangkok, Thailand og i udlandet.",
            image: "/chorn-images/home/chorn-workplace-001.jpg"
        },
        {
            title: "Fleksible Arbejdsrum",
            description: "Fleksible arbejdspladser efter kundens behov, beliggende i Bangkok, Thailand, tæt på BTS- og MRT-stationer, vigtige vartegn og førende indkøbscentre som Siam Paragon, Central World og ICON Siam. Klar til fjernarbejde, hybridarbejde eller arbejde på stedet i Chiang Mai, Bangkok, Thailand og i udlandet.",
            image: "/chorn-images/home/chorn-workplace-002.jpg"
        },
        {
            title: "Fleksible Arbejdsrum",
            description: "Fleksible arbejdspladser efter kundens behov, beliggende i Bangkok, Thailand, tæt på BTS- og MRT-stationer, vigtige vartegn og førende indkøbscentre som Siam Paragon, Central World og ICON Siam. Klar til fjernarbejde, hybridarbejde eller arbejde på stedet i Chiang Mai, Bangkok, Thailand og i udlandet.",
            image: "/chorn-images/home/chorn-workplace-003.jpg"
        },
        {
            title: "Fleksible Arbejdsrum",
            description: "Fleksible arbejdspladser efter kundens behov, beliggende i Bangkok, Thailand, tæt på BTS- og MRT-stationer, vigtige vartegn og førende indkøbscentre som Siam Paragon, Central World og ICON Siam. Klar til fjernarbejde, hybridarbejde eller arbejde på stedet i Chiang Mai, Bangkok, Thailand og i udlandet.",
            image: "/chorn-images/home/chorn-workplace-004.jpg"
        },
        {
            title: "Fleksible Arbejdsrum",
            description: "Fleksible arbejdspladser efter kundens behov, beliggende i Bangkok, Thailand, tæt på BTS- og MRT-stationer, vigtige vartegn og førende indkøbscentre som Siam Paragon, Central World og ICON Siam. Klar til fjernarbejde, hybridarbejde eller arbejde på stedet i Chiang Mai, Bangkok, Thailand og i udlandet.",
            image: "/chorn-images/home/chorn-workplace-005.jpg"
        },
        {
            title: "Fleksible Arbejdsrum",
            description: "Fleksible arbejdspladser efter kundens behov, beliggende i Bangkok, Thailand, tæt på BTS- og MRT-stationer, vigtige vartegn og førende indkøbscentre som Siam Paragon, Central World og ICON Siam. Klar til fjernarbejde, hybridarbejde eller arbejde på stedet i Chiang Mai, Bangkok, Thailand og i udlandet.",
            image: "/chorn-images/home/chorn-workplace-006.jpg"
        },
    ],
    global: {
        title: "International Rejse",
        description: "CHORN har erfaring med internationale rejser til træning, internationale arbejdsstudierejser både i ASEAN og USA, afholdelse af workshops og er klar til både indenlandske og internationale rejser. CHORN tilbyder fjernarbejde, hybridarbejde og kontorbaseret arbejde i Chiang Mai, Bangkok, Thailand og internationalt.",
        image: "/chorn-images/gallery/chorn-global-001.jpg"
    },
    globalSlides: [
        {
            title: "",
            description: "",
            image: "/chorn-images/gallery/chorn-global-002.jpg"
        },
        {
            title: "",
            description: "",
            image: "/chorn-images/gallery/chorn-global-003.jpg"
        },
        {
            title: "",
            description: "",
            image: "/chorn-images/gallery/chorn-global-004.jpg"
        },
        {
            title: "",
            description: "",
            image: "/chorn-images/gallery/chorn-global-005.jpg"
        }
    ]
};
