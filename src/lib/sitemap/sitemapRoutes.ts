...
import { getFrontendRoutes } from "@/lib/platform-content/frontendRoutes";
import { getFullstackRoutes } from "@/lib/platform-content/fullstackRoutes";
import {
...
const NON_INDEXABLE_STATIC_PATHS = new Set(["/404/", "/smart-mobility/mts/"]);
const DYNAMIC_ROUTE_RESOLVERS: Record<string, DynamicRouteResolver> = {
  ...
  "/technical-expertise/front-end-developer/[slug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/technical-expertise/front-end-developer/[slug]/",
    );

    return (await getFrontendRoutes()).map((route) => ({
      path: `/technical-expertise/front-end-developer/${route.slug}/`,
      sourceFile,
    }));
  },
  "/technical-expertise/full-stack-developer/[slug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/technical-expertise/full-stack-developer/[slug]/",
    );

    return (await getFullstackRoutes()).map((route) => ({
      path: `/technical-expertise/full-stack-developer/${route.slug}/`,
      sourceFile,
    }));
  },
  ...
  "/future-civilization/[eraSlug]/[slug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/future-civilization/[eraSlug]/[slug]/",
    );

    return (await getFutureRoadmapItemStaticParams()).map(
      ({ eraSlug, slug }) => ({
  ...
  "/business-opportunities/[nodeSlug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/business-opportunities/[nodeSlug]/",
    );
  ...
  "/industries/[nodeSlug]/[subNodeSlug]/[problemSlug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/industries/[nodeSlug]/[subNodeSlug]/[problemSlug]/",
    );
  ...
  "/business-opportunities/[nodeSlug]/[businessSlug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/business-opportunities/[nodeSlug]/[businessSlug]/",
    );
  ...
  "/future-solutions/[eraSlug]/[eraItemSlug]/": async () => {
    const sourceFile = getSourceFileForRoute(
      "/future-solutions/[eraSlug]/[eraItemSlug]/",
    );
...
function getUniqueSlugs(items: Array<{ slug?: string }>): string[] {
  return Array.from(
    new Set(items.map((item) => item.slug).filter(Boolean)),
  ) as string[];
}
...
async function getDiscoveredRoutes(): Promise<SitemapRoute[]> {
  ...
  return dedupeRoutes(routes).sort((first, second) =>
...
export async function getLocalizedSitemapEntries(
